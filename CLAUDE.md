...
## 🔬 Current Testing Focus (2025-12-05)

**Problem:** Agent SDK субагенты слишком долго выполняют простые задачи (40+ секунд для создания простого куба)

**Этап:** Анализ производительности и оптимизация промптов агентов

**Что тестируем:**
1. **Скорость выполнения задач** - измеряем время от запроса до создания файла
2. **Количество операций** - считаем лишние read/write/tool calls
3. **Thinking time** - находим где агент долго "думает" без действий
...
**Expected результат после оптимизации:** < 15 секунд для создания простого объекта
