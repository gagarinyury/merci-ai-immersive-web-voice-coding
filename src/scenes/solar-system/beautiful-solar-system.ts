/**
 * Beautiful Solar System - Advanced version with procedural textures, PBR materials, and real orbital motion
 *
 * Features:
 * - 8 planets with realistic orbits around the Sun
 * - Procedurally generated textures with noise patterns
 * - PBR materials (metalness, roughness, bump maps)
 * - Point light from the Sun
 * - Sun corona with pulsing animation
 * - 3000 stars with varying sizes and colors
 * - Orbital trail rings for each planet
 * - Multi-layered Saturn rings
 * - Vite HMR cleanup for hot reload
 */
...
// Create advanced procedural texture with Perlin-like noise
...
// Planet data with advanced materials
const planets = [
...
// Saturn rings (enhanced with multiple layers)
...
const animate = () => {
  ...
  planetMeshes.forEach((planet, index) => {
    ...
    // Update Saturn rings position (Saturn is index 5)
    if (index === 5) {
...
console.log('🌌✨ BEAUTIFUL Solar System created!', {
  planets: 8,
  stars: 3000,
  features: 'Bump maps, metalness, roughness, orbital trails, Saturn rings, sun corona, point light'
});
...
if (import.meta.hot) {
  ...
  import.meta.hot.dispose(() => {
    console.log('🧹 Cleaning up solar system');
    if (animationId) {
