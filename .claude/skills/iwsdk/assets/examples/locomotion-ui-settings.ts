/**
 * Advanced Locomotion Settings Panel (from IWSDK locomotion example)
 *
 * Real working code from Meta's IWSDK repository.
 * Demonstrates:
 * - Complex UI state management
 * - Dynamic button styling based on selection
 * - Accessing LocomotionSystem config
 * - Toggle panel visibility with gamepad
 * - Audio feedback on button clicks
 * - Multiple queries with 'where' filters
 *
 * Source: iwsdk/examples/locomotion/src/panel.js
 */
...
export class SettingsSystem extends createSystem({
  ...
  init() {
    // Setup settings panel when loaded
      ...
      buttonIds.forEach((buttonId) => {
        ...
        if (button) {
          button.addEventListener('click', () => {
            ...
            let configKey = null;
            for (const [key, selectedButtonId] of Object.entries(configOptions)) {
            ...
            if (configKey) {
              ...
              // Update LocomotionSystem config
              if (value !== undefined) {
              ...
              updateButtonStyling();
              AudioUtils.play(entity); // Play click sound
  ...
  update() {
    // Toggle panel with left controller select button
